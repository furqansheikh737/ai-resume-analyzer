...
export const AnalysisReport = ({ data, onReset }: AnalysisProps) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div ref={reportRef} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl">
        <div className="flex flex-col md:flex-row items-center gap-10">
          <div className="w-48 h-48 md:w-56 md:h-56">
            <CircularProgressbar
              ...
              styles={buildStyles({
                textSize: '22px',
                pathColor: data.score > 70 ? '#22c55e' : data.score > 40 ? '#f59e0b' : '#ef4444',
                textColor: 'currentColor', // Yeh text color theme ke hisab se lega
                trailColor: '#1e293b',
                pathTransitionDuration: 2,
              })}
            />
            <p className="text-center mt-4 font-extrabold text-slate-500 uppercase tracking-widest text-[10px]">ATS Score</p>
          </div>

          <div className="flex-1 space-y-4 text-center md:text-left">
            <h2 className="text-4xl font-black tracking-tight dark:text-white text-slate-900">Analysis Result</h2>
            <p className="text-slate-500 dark:text-slate-400 text-lg">
              Your profile has been evaluated against current 2026 recruitment standards.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-3">
              {data.strengths.map((s, i) => (
                <li key={i} className="p-4 rounded-2xl bg-green-50/50 dark:bg-green-500/5 border border-green-100 dark:border-green-500/10 text-slate-700 dark:text-slate-300 text-sm font-medium">
                  {s}
                </li>
              ))}
            </ul>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-3">
              {data.improvements.map((imp, i) => (
                <li key={i} className="p-4 rounded-2xl bg-amber-50/50 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/10 text-slate-700 dark:text-slate-300 text-sm font-medium">
                  {imp}
                </li>
              ))}
            </ul>
